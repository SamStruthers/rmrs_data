...
all_stage_Q_15min <- stage_Q_15min %>%
  ...
  mutate(site = case_when(site == "LBEA1" ~ "LBEA",
                          site == "LBEA2" ~ "LBEA",
                          TRUE ~ site))%>%
  select(site, DT_round_mst, Q_cfs,sensor_stage_cm, adjustment_cm, corrected_stage_cm, extrapolated_Q_cfs)
```
...
ggplot(filter(stage_Q_daily, site_code == "LBEA2"), aes(date, Q_cfs, color = site_code))+
  geom_point()+
    geom_point(data = filter(Q_2022, site_code == "FISH"), aes(x= date, y = q_cfs), color = "blue")+
  geom_point(data = filter(Q_2023, site == "FISH"), aes(x= date, y = q_cfs), color = "blue")+
  #facet_wrap(~year(date), scales = "free_x")+
  theme_bw()+
  theme(legend.position = "bottom")+
  labs(x = "Date", y = "Discharge (cfs)", color = "Site Code")

ggplot(filter(stage_Q_15min, site_code == "LBEA2"), aes(DT))+
  geom_point(aes(y = sensor_stage_cm))+
   geom_point(aes(y = corrected_stage_cm), color = "red")+
   geom_point(data = filter(manual_measurements, site_code == "LBEA" ), aes(y = H_cm), color = "blue")
...
all_data <- map_dfr(sites, create_all_data)%>%
  ...
  filter(rowSums(!is.na(select(., -site, -DT_round_mst, -precip_site))) > 0)%>%
...
all_data_daily <- all_data%>%
  ...
  summarise(
    ...
    mean_cdom_ave = mean(cdom_ave, na.rm = TRUE),
    mean_cdom = mean(cdom, na.rm = TRUE),
    mean_turb_ave = mean(turb_ave, na.rm = TRUE),
    mean_turb = mean(turb, na.rm = TRUE),
    mean_sensor_stage_cm = mean(sensor_stage_cm, na.rm = TRUE),
    mean_corrected_stage_cm = mean(corrected_stage_cm, na.rm = TRUE),
    mean_Q_cfs = mean(Q_cfs, na.rm = TRUE),
    mean_extrapolated_Q_cfs = mean(extrapolated_Q_cfs, na.rm = TRUE), 
    mean_precip_in = sum(precip_in, na.rm = TRUE)
  )%>%
  ungroup()%>%
...
all_data_daily_long <- all_data_daily%>%
  pivot_longer(cols = c(mean_do_mgL:mean_precip_in), names_to = "parameter", values_to = "value")%>%
  filter(!is.nan(value) & !is.na(value))
...
ggplot(filter(all_data_daily, site == "FISH"), aes(x = doy)) +
   geom_point(aes(y = mean_minidot_temp_c, color = "Minidot Temp", shape = year), size = 3) +
   geom_point(aes(y = mean_ISCO_temp_c, color = "ISCO Temp", shape = year), size = 3) +
...
ggplot(data = filter(all_data_long, parameter %in% c("precip_in", "turb_ave",  "sc_us")& site == "SAWM"), aes(x= DT_round_mst, y = value, color = site))+
  geom_point()+
  facet_wrap(~parameter, scales = "free_y")

# view daily data
ggplot(data = all_data_daily_long, aes(x= date, y = value, color = site))+
  geom_point()+
  facet_wrap(~parameter,scales = "free_y")
